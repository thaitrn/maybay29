...
app.get('/healthz', async () => ({ ok: true, version: process.env.npm_package_version ?? '1.0.0', db: db.pragma('journal_mode', { simple: true }) }));
...
app.post('/v2/players', async (req, reply) => {
  ...
  db.prepare(`INSERT INTO player (id, display_name) VALUES (?, ?)
    ON CONFLICT(id) DO UPDATE SET display_name = excluded.display_name`).run(player_id, name);
...
app.post('/v2/scores', async (req, reply) => {
  ...
  db.prepare('INSERT INTO score (player_id, value, duration_s) VALUES (?, ?, ?)').run(player_id, v, dur);
...
app.get('/v2/leaderboard', async (req) => {
  ...
  const rows = db.prepare(`
    ...
    GROUP BY s.player_id ORDER BY value DESC, created_at ASC LIMIT ?`).all(limit);
...
app.listen({ port: PORT, host: '0.0.0.0' }).then(() => console.log(`maybay29 BE on :${PORT}`));
